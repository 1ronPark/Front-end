  ...
  >({
    mutationFn: (body) =>
      fetchRequest<CreateResponse<CommentCreated>, { content: string }>({
        method: "POST",
        endpoint: import.meta.env.VITE_API_ITEMS_COMMENT_POST_ENDPOINT.replace(
          ":id",
          String(itemId)
        ),
        body,
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({
        queryKey: ["GET", `VITE_API_ITEMS_ENDPOINT/${itemId}`, ""],
      });
    },
  });
};
  ...
  return useMutation<DeleteResponse, Error, { commentId: number }>({
    mutationFn: ({ commentId }) =>
      fetchRequest<DeleteResponse>({
        method: "DELETE",
        endpoint: import.meta.env.VITE_API_ITEMS_DELETE_COMMENT.replace(
          ":id",
          String(commentId)
        ),
      }),
